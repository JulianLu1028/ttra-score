...
2. 新專案依序在 SQL Editor 執行 supabase/migrations/001_competition.sql、002_heats_and_privacy.sql、003_academic.sql、004_participant_numbering.sql，各一次。已套用前三個 migration 的正式專案只執行 004；不要重跑舊 migration。002 會移除既有學校欄及稽核中的學校資料，並將既有參賽者暫設第 1 梯；004 會要求 A001／B001／C001／D001 格式及正確組別前綴，執行前須先修正任何舊格式編號。之後的新匯入必須明確填寫梯次，組別由工作台目前選擇自動帶入。
