...
describe("CSV", () => {
  ...
  it("拒絕含學校及缺少梯次的舊版 CSV", () => {
    expect(() =>
      parseTeams("參賽編號,姓名,學校／單位,組別\n001,陳宥安,學校,power"),
    ).toThrow("新版範本");
    expect(() => parseTeams("參賽編號,姓名,組別\n001,陳宥安,power")).toThrow(
      "梯次",
    );
  });
  it("拒絕重複及錯誤組別", () =>
    expect(() =>
      parseTeams("team_number,name,category_id,heat\n1,A,power,1\n1,B,power,2"),
    ).toThrow("重複"));
  it("匯出防止試算表公式注入", () =>
